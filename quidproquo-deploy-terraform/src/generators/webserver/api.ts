...
export const apiGenerator: ResourceGenerator = {
  ...
  generate(setting: QpqConfigSettingJson, ctx: GeneratorContext): GeneratedResource[] {
    ...
    const routes = routeSettings.map((r) => {
      ...
      const routePath = readString(r, 'path') ?? '';
      const routeKey = `${method.toLowerCase()}_${routePath.replace(/^\//, '').replace(/[^a-z0-9]+/gi, '_')}`;
      ...
      const routeLabel = buildModuleLabel('route', routeKey);
      ...
      if (artifactsDir) {
        return {
          ...
          runtime: r.runtime,
          options: r.options ?? {},
          source_code_path: path.join(artifactsDir, routeKey),
        };
      }
